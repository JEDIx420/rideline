...
export class FallbackEngineAudio {
  ...
  public start(): void {
    ...
    (this.distortionNode as unknown as { curve: Float32Array | null }).curve = this.makeDistortionCurve(22);
    ...
    // 2. Harmonic (camshaft / intake whine)
  ...
  public update(
    ...
    const targetCutoff = baseCutoff + throttle * 4800 + rpmRatio * 3200;
    const finalCutoff = mix ? Math.min(mix.lowpassCutoffHz, targetCutoff) : targetCutoff;
    this.filterNode?.frequency.setTargetAtTime(finalCutoff, t, 0.04);

    // Volume calculation
    const baseVolume = 0.20 + throttle * 0.24 + rpmRatio * 0.16;
    const cameraVolumeMod = isCockpit ? (mix ? mix.intakeVolume : 0.85) : (mix ? mix.exhaustVolume : 1.0);
    this.gainNode?.gain.setTargetAtTime(baseVolume * cameraVolumeMod, t, 0.04);
  }
